...
namespace ITG.Brix.Users.IntegrationTests.Infrastructure.Bases
{
    public static class RepositoryTestsHelper
    {
        ...
        private static void CollectionCreate(string collectionName)
        {
            ...
            database.GetCollection<User>(collectionName).Indexes.CreateOneAsync(new CreateIndexModel<User>(Builders<User>.IndexKeys.Ascending(_ => _.Login.Value), new CreateIndexOptions() { Unique = true }), cancellationToken: CancellationToken.None).ConfigureAwait(false);
        }
